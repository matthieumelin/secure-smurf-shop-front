...
export default function AdminUsers() {
    ...
    const haveResult = users && users.filter((user) => user.email.toLowerCase().includes(searchedUser.toLowerCase()) || user.username.toLowerCase().includes(searchedUser.toLowerCase())).length > 0;
                ...
                <WrapperRight>
                    ...
                    <Container>
                        ...
                        <ContainerBody>
                            <List>
                                <ListHeader>
                                    <ListHeaderTitle>User List ({users.length})</ListHeaderTitle>
                                    <ListHeaderSearch>
                                        <ListHeaderSearchIcon src={`${process.env.PUBLIC_URL}/assets/icons/search.svg`} alt="Search User" />
                                        <ListHeaderSearchInput type='text' placeholder="Search User" onChange={onSearchUsers} />
                                    </ListHeaderSearch>
                                </ListHeader>
                                <ListBody haveResult={haveResult}>
                                    {searchedUser ? (
                                        haveResult ? (
                                            users.map((user) => renderUserList(user))
                                        ) : (
                                            <ListBodyNoMatch>No user found..</ListBodyNoMatch>
                                        )
                                    ) : (
                                        currentRecords && currentRecords.map((currentRecord) => renderUserList(currentRecord))
                                    )}
                                </ListBody>
                            </List>
